% Min and Max values for the optimization procedure parameters

%% C1 - x position of 1st Mean Line control point
min_c1 = 0.1; %default = 0.01;
max_c1 = 0.5; %default = 0.96;
step_c1 = 0.1;

%% C2 - x position of 2nd Mean Line control point
min_c2 = 0.5; %default = 0.02;
max_c2 = 0.8; %default = 0.97;
step_c2 = 0.1;

%% C3 - y position of 1st Mean Line control point
min_c3 = 0; %default = -0.074;
max_c3 = 0.3; %default = 0.247;
step_c3 = 0.1;

%% C4 - y position of 2nd Mean Line control point
min_c4 = 0; %default = -0.102;
max_c4 = 0.3; %default = 0.206;
step_c4 = 0.1;

%% x_t - chordwise location of maximum thickness
min_x_t = 0.2; %default = 0.2002;
max_x_t = 0.5; %default = 0.4813;
step_x_t = 0.1;

%% T - maximum thickness
min_T = 0.01; %default = 0.0246;
max_T = 0.15; %default = 0.3227;
step_T = 0.1;

%% rho_bar - adimensional rho -> leading edge radius
min_rho = 0.175;
max_rho = 1; %default = 1.4944;
step_rho = 0.1;

%% beta_TE_bar - trailing edge boat-tail angle
min_beta_TE = 1; %default = 0.1452;
max_beta_TE = 3; %default = 4.8724;
step_beta_TE = 0.1;

%% AoA main and flap - Angle of attack
min_AoA_main = -5;
max_AoA_main = 25;
step_AoA_main = 1;

min_AoA_flap = -30;
max_AoA_flap = -70;
step_AoA_flap = -1;

%% Slot Definition -- CHECK AXES DEFINITION OF IGP
min_x = -0.01;%default = -0.02;
max_x = 0.02; % default = 0.016;
step_x = 0.01;

min_y = 0.01;
max_y = 0.016;
step_y = 0.1;

%% Flap Chord - (fraction of main airfoil chord, namely c = 1)
min_c_flap = 0.2;
max_c_flap = 0.4;
step_c_flap = 0.1;


%% ------------------------------------------------------------------ %%
%% ----------------- NON MODIFICARE OLTRE --------------------------- %%
%% ------------------------------------------------------------------ %%


%% IGP - Main Profile
c1 = [min_c1:step_c1:max_c1];
c2 = [min_c2:step_c2:max_c2];
c3 = [min_c3:step_c3:max_c3];
c4 = [min_c4:step_c4:max_c4];
x_t = [min_x_t:step_x_t:max_x_t];
T = [min_T:step_T:max_T];
rho = [min_rho:step_rho:max_rho];
beta_TE = [min_beta_TE:step_beta_TE:max_beta_TE];

%% IGP - Flap Profile
c1 = [min_c1:step_c1:max_c1];
c2 = [min_c2:step_c2:max_c2];
c3 = [min_c3:step_c3:max_c3];
c4 = [min_c4:step_c4:max_c4];
x_t = [min_x_t:step_x_t:max_x_t];
T = [min_T:step_T:max_T];
rho = [min_rho:step_rho:max_rho];
beta_TE = [min_beta_TE:step_beta_TE:max_beta_TE];

%% AoA
AoA_main = [min_AoA_main:step_AoA_main:max_AoA_main];
AoA_flap = [min_AoA_flap:step_AoA_flap:max_AoA_flap];

%% Slot Definition
x = [min_x:step_x:max_x];
y = [min_y:step_y:max_y];

%% Flap Chord (relative to Main Profile Chord c = 1)
c_flap = [min_c:step_c:max_c];


%NOTE: If DRS on, AoA of the Main profile does not change. AoA of the flap
%tends towars zero-drag incidence. X distance is the same, Y distance
%becomes --> Y = 0.095 m.